...
def structure_components(component_library, component_class, component_type):
    ...
    for i in range(len(component_library['Component number'])):
        ...
        # Ensure component_type[class_idx] is a list
        if isinstance(component_type[class_idx], list):
            type_name = component_type[class_idx][type_idx]
        else:
            type_name = component_type[class_idx]
...
def main():
    # ----------------------------
    # 1. Define Component Classes and Types
    # ----------------------------
    component_class = ["VehicleBody", "GearSystems", "EnergyConverters", "EnergyStorage", "Controller"]
    component_type = [
        ["Vehicle body"],  # Ensured as a list for consistency
        ["Final Drive", "Simple transmission", "Multispeed gearbox"], 
        ["Electric motor 1"], 
        ["Battery"], 
    ...
    print("Components:")
    for idx, (cls, typ) in enumerate(components):
        print(f"Component {idx}: Class={cls}, Type={typ}")
    print("\n")

    # ----------------------------
    # 4. Instantiate the CP Model
    # ----------------------------
    model = cp_model.CpModel()

    # ----------------------------
    # 5. Define Decision Variables (DSM and Usage)
    # ----------------------------
    DSM = {}
    ...
    # ----------------------------
    # 6. Create Mappings from Indices to Classes and Types
    # ----------------------------
    ...
    power_types = get_power_types(component_library, component_class, component_type)
    ...
    # ----------------------------
    # 7. Define Component Groups Based on Classes and Types
    # ----------------------------
    ...
    # ----------------------------
    # 8. Link DSM Variables to Usage Variables
    # ----------------------------
    for i in range(num_components):
        for j in range(i + 1, num_components):
            ...
            # These constraints ensure that DSM[i,j] can be 1 only if both used[i] and used[j] are 1

    # Additionally, ensure that used[i] is 1 if any DSM connections involve i
    for i in range(num_components):
        connected = []
        for j in range(num_components):
            if i < j:
                connected.append(DSM[i, j])
            elif j < i:
                connected.append(DSM[j, i])
        if connected:
            # If any connection exists, used[i] must be 1
            model.AddMaxEquality(used[i], connected)
        else:
            # If no possible connections, allow used[i] to be 0
            model.Add(used[i] == 0)

    # ----------------------------
    # 9. Enforce Required Components to be Used
    # ----------------------------
    ...
    print("self.solution_count: ", solution_printer.solution_count)
